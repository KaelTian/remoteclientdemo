<template>
    <div class="plc-points-monitor">
        ...
        <h2>动态PLC点位测试类</h2>
        <div class="data-list">
            <div class="data-item" v-for="(value, key) in pointsData" :key="key"
...
export default {
    ...
    setup() {
        ...
        const handleDataReceived = (newData) => {
            ...
            // 更新最后更新时间
            lastUpdateTime.value = new Date().toLocaleString('zh-CN', { hour12: false, year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', fractionalSecondDigits: 3 });
        }
        const config = {
            url: "http://localhost:8091/signalr?user=Receiver1&group=OmronDevice1PLCGroup",
            receiveMethod: "ReceivePLCPointsEvent",
            sendMethod: "Message",
            callbackUser:"CollectorOmron",
            callbackMethod:"UpdatePLCPointsEvent",
            withCredentials: false
        };
        const signalrService = new ConfigurableSignalrService(config);
        onMounted(async () => {
            try {
